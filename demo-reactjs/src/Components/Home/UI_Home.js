import React from 'react';
import { makeStyles } from '@material-ui/core/styles';
import Container from '@material-ui/core/Container';
import { CssBaseline, Grid } from '@material-ui/core';
import Button from '@material-ui/core/Button';
import Typography from '@material-ui/core/Typography';
import Card from './UI_Card';
import Footer from './UI_Footer';
import CardContent from './UI_CardContent';

const useStyle = makeStyles((theme) => ({
    root: {
        height: 'auto',
        display: 'block',
        backgroundSize: 'cover',
        margin: '0px',
        padding: '0px',
    },

    root1: {
        margin: '0px',
        padding: '0px',
        background: 'url("./img/banner-bg.jpg") no-repeat fixed',
        height: 'auto',
        display: 'flex',
        backgroundSize: 'cover',
        paddingBottom: '12%',
        [theme.breakpoints.down('md')]: {
            paddingBottom: '2%',
        },
        paddingTop: '150px',
        [theme.breakpoints.down('md')]: {
            paddingTop: '100px',
        }
    },

    root2: {
        margin: '0px',
        padding: '0px',
        height: 'auto',
        display: 'flex',
        backgroundSize: 'cover',
        width: 'auto',
    },

    root3: {
        margin: '0px',
        padding: '0px',
        background: 'url("./img/feature-bg.png") no-repeat fixed',
        height: 'auto',
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
        backgroundSize: 'cover',
        paddingTop: '4%',
        [theme.breakpoints.down('md')]: {
            paddingTop: '50px',
        },
        paddingBottom: '5%',
        [theme.breakpoints.down('md')]: {
            paddingBottom: '30px'
        }
    },


    wrapper: {
        display: 'block',
        [theme.breakpoints.up('md')]: {
            paddingTop: '',
        },
        [theme.breakpoints.down('md')]: {
            paddingTop: '0px',
        }

    },

    wrapperLeft: {
        [theme.breakpoints.down('md')]: {
            paddingLeft: '10px'
        }
    },

    cards: {
        marginLeft: '50px',

        [theme.breakpoints.down('sm')]: {
            marginLeft: '0px'
        },
        [theme.breakpoints.between('sm', 'md')]: {
            marginLeft: '15px'
        },
        [theme.breakpoints.between('md', 'lg')]: {
            marginLeft: '25px'
        }

    },

    card: {
        display: 'flex',
        justifyContent: 'center',
        [theme.breakpoints.down('md')]: {
            paddingLeft: '32px'
        },
        [theme.breakpoints.down('lg')]: {
            marginBottom: '30px',
        }
    },

    cardContents: {
        [theme.breakpoints.down('sm')]: {
            paddingLeft: '5px',
        },

        [theme.breakpoints.up('sm')]: {
            paddingLeft: '20px',
        },

        [theme.breakpoints.up('md')]: {
            paddingLeft: '50px',
            paddingRight: '70px'
        }
    },

    btn: {
        width: '150px',
        height: '38px',
        backgroundColor: '#ffffff',
        color: '#343434',
        marginTop: '20px',
        '&:hover': {
            backgroundColor: 'transparent',
            border: 'solid 1px white',
            color: 'white',
        }
    }
}));

export default function Home(props) {
    const classes = useStyle();

    return (
        <div className={classes.root}>
            <CssBaseline />
            {/* Phần 1 */}
            <div className={classes.root1}>
                <Container className={classes.wrapper}>
                    <Grid container>
                        <Grid item xs={12} sm={12} md={6} lg={6}>
                            <div className={classes.wrapperLeft}>
                                <Typography variant="h1" style={{ lineHeight: '60px' }}>Improved <br />Production level <br />with Robotics</Typography>
                                <Typography variant="p" style={{ lineHeight: '70px', color: 'white', textTransform: 'uppercase', marginRight: '15px' }}>Everyone wants the innovation through robotics</Typography>
                                <br />
                                <Button className={classes.btn} href="#"><Typography variant="p">View Details</Typography></Button>
                            </div>
                        </Grid>
                        <Grid item xs={12} sm={12} md={6} lg={6}>
                            <div>
                                <img style={{ maxHeight: '100%', maxWidth: '100%' }} src="./img/banner-img.png" alt="banner-img.png"></img>
                            </div>
                        </Grid>
                    </Grid>
                </Container>
            </div>
            {/* end phần 1 */}

            {/* phần 2 */}
            <div className={classes.root2}>
                <Container style={{ marginTop: '50px', display: 'flex', flexDirection: 'column', paddingLeft: '50px' }}>
                    <Grid container>
                        <Grid item xs={12} sm={12} md={12} lg={12} style={{ marginBottom: '90px', marginTop: '90px', textAlign: 'center', lineHeight: '50px' }}>
                            <Typography style={{ fontWeight: '500' }} variant="h3">Featured Robotics Products to Show</Typography>
                            <Typography variant="p" style={{ color: '#7B7B91' }}>
                                Who are in extremely love with eco friendly system.
							</Typography>
                        </Grid>
                    </Grid>
                    <Grid container>
                        <Grid item xs={11} className={classes.cards}>
                            <Grid container>
                                <Grid item xs={12} sm={6} md={6} lg={3} className={classes.card}><Card imga="./img/p1.png" name="The Upper Eye" note="
										Who are in extremely love with eco friendly system."/></Grid>
                                <Grid item xs={12} sm={6} md={6} lg={3} className={classes.card}><Card imga="./img/p2.png" name="The Crab Wheel" note="
										Who are in extremely love with eco friendly system."/></Grid>
                                <Grid item xs={12} sm={6} md={6} lg={3} className={classes.card}><Card imga="./img/p3.png" name="The Plug Ninja" note="
										Who are in extremely love with eco friendly system."/></Grid>
                                <Grid item xs={12} sm={6} md={6} lg={3} className={classes.card}><Card imga="./img/p4.png" name="The Controller" note="
										Who are in extremely love with eco friendly system."/></Grid>
                            </Grid>
                        </Grid>
                        <Grid item xs={1}></Grid>
                    </Grid>
                </Container>
            </div>
            {/* end phần 2 */}

            {/* phần 3 */}
            <div className={classes.root3}>
                <Container>
                    <Grid>
                        <Grid item xs={12} sm={12} md={12} lg={12} style={{ marginBottom: '40px', marginTop: '40px', textAlign: 'center', lineHeight: '50px' }}>
                            <Typography style={{ fontWeight: '500', color: 'white' }} variant="h3">Some Features that Made us Unique</Typography>
                            <Typography variant="p" style={{ color: '#7B7B91', color: 'white' }}>
                                Who are in extremely love with eco friendly system.
							</Typography>
                        </Grid>
                    </Grid>
                    <Grid container className={classes.cardContents}>
                        <Grid item xs={12} sm={6} md={6} lg={4}>
                            <CardContent title="Expert Technicians" />
                        </Grid>
                        <Grid item xs={12} sm={6} md={6} lg={4}>
                            <CardContent title="Expert Technicians" />
                        </Grid>
                        <Grid item xs={12} sm={6} md={6} lg={4}>
                            <CardContent title="Expert Technicians" />
                        </Grid>
                        <Grid item xs={12} sm={6} md={6} lg={4}>
                            <CardContent title="Expert Technicians" />
                        </Grid>
                        <Grid item xs={12} sm={6} md={6} lg={4}>
                            <CardContent title="Expert Technicians" />
                        </Grid>
                        <Grid item xs={12} sm={6} md={6} lg={4}>
                            <CardContent title="Expert Technicians" />
                        </Grid>
                    </Grid>
                </Container>
            </div>
            {/* end phần 3 */}
            <Footer />
        </div>
    );
}